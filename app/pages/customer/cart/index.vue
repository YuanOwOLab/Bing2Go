<template>
    <v-container>
        ...
        <v-row v-else align="start">
                                    ...
                                    <v-avatar size="64" rounded="lg" class="mr-4">
                                        <v-img :src="item.image" cover>
                                            <template #error>
                                                ...
                                                >
                                                    <span class="text-h6 font-weight-bold" style="color: #e0b4c3"
                                                        >???</span
                                                    >
                                                </v-sheet>
                                            </template>
            ...
            <v-col cols="12" md="4">
                <v-card v-if="cartStore.items.length > 0" elevation="2" rounded="lg" class="summary-card">
...
const fetchDeliveryInfo = async () => {
    ...
    const restaurantIds = Array.from(new Set(cartStore.items.map((item) => item.restaurantId).filter(Boolean)));
...
watch([() => cartStore.deliveryAddress, () => cartStore.items], scheduleDeliveryInfoUpdate, { deep: true });
</script>
