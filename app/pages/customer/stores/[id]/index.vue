<template>
    <div>
        <v-img :src="store?.image" height="400px" cover class="w-100 position-relative store-cover">
            <template #error>
                <v-sheet color="grey-lighten-2" class="d-flex align-center justify-center fill-height">
        ...
        <v-container class="px-4">
            <v-card class="info-card mx-auto px-6 py-5" max-width="950" elevation="8">
        ...
        <v-container class="menu-info pt-0 mt-n10">
            ...
            <v-row>
                <v-col v-for="item in store?.menu" :key="item._id" cols="12" sm="6" md="4" lg="4" class="pa-2">
                    <MenuItemCard :item="item" @open-add-dialog="openDialog" />
                </v-col>
            </v-row>
        </v-container>

        <AddToCartDialog v-model="isDialogOpen" :item="selectedItem" @add-to-cart="handleAddToCart" />
    </div>
</template>
...
const { data: apiResponse, refresh } = useFetch<ApiResponse>(`/api/restaurants/${storeId}`);
