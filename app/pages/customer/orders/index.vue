    ...
    <v-container>
        ...
        <v-window v-model="tab">
            <v-window-item value="inProgress">
                <v-row>
                    <v-col v-for="order in inProgressOrders" :key="order.id" cols="12" md="6" lg="4">
                        <OrderCard
                            role="customer"
                            :order="order"
                            :path="`/customer/order-state/${order.id}`"
                            :has-notification="
                                notificationStore.hasMessage(order.id) || notificationStore.hasStatusUpdate(order.id)
                            "
                        />
                    </v-col>
                    <v-col v-if="inProgressOrders.length === 0" cols="12">
                        <p class="text-center text-medium-emphasis mt-10">沒有未完成的訂單</p>
                    </v-col>
                </v-row>
            </v-window-item>
            ...
            <v-window-item value="completed">
                <v-row>
                    ...
                    <v-col v-if="completedOrders.length === 0" cols="12">
                        <p class="text-center text-medium-emphasis mt-10">沒有已完成的訂單</p>
                    </v-col>
                </v-row>
            </v-window-item>
        </v-window>
...
const allOrders = computed(() =>
    rawOrders.value.map((order) => {
        const restaurantNames = Array.from(new Set(order.items.map((item) => item.restaurant.name))).join(", ");
...
const inProgressOrders = computed(() => allOrders.value.filter((order) => inProgressStatuses.includes(order.status)));
const completedOrders = computed(() => allOrders.value.filter((order) => completedStatuses.includes(order.status)));
