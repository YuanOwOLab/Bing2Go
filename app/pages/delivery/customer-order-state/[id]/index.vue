        ...
        <v-row justify="center">
            <v-col cols="12" md="10" lg="8">
                ...
                <template v-else key="content">
                    ...
                    <v-card flat border rounded="lg" class="mb-6">
                        <v-card-title class="text-h6 font-weight-bold"> 外送地圖（示意） </v-card-title>
                        <v-card-text>
                            <DeliveryMap
                    ...
                    >
                        <v-btn color="primary" block size="large" @click="navigateTo(`/chat/${orderId}`)">
...
const { driverPosition: courierPosition, sendLocation, disconnect: stopTrackingSocket } = useOrderTracking(orderId);
...
const shouldShareLocation = computed(
    () => userStore.currentRole === "delivery" && orderData.value?.deliveryPerson?._id === userStore.info?.id,
);
