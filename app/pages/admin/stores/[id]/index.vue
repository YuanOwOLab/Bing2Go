<template>
    <v-container class="py-8">
        ...
        <v-alert v-else-if="error" type="error"> 載入資料時發生錯誤：{{ error?.message || "未知錯誤" }} </v-alert>
                        ...
                        <v-card-text class="pa-4 pa-md-8">
                                    ...
                                    <div class="image-wrapper">
                                        ...
                                        >
                                            <template #placeholder>
                                                ...
                                                >
                                                    <v-icon color="grey-lighten-1" size="50"
                            ...
                            <v-slide-y-transition group tag="div">
                                <div
                                    ...
                                    class="mb-6"
                                >
                                    <v-card flat border rounded="lg" class="menu-item-card">
                                        <v-card-text>
                                            <v-row>
                                                <v-col cols="12" sm="4" md="3">
                                                    ...
                                                    <div class="menu-image-wrapper mx-auto">
                                                        ...
                                                        >
                                                            ...
                                                            <div
                                                                class="menu-image-overlay d-flex flex-column align-center justify-center"
                                                            >
                                                                <v-icon size="32" color="white">mdi-camera</v-icon>
                                                                <span class="text-caption text-white mt-1"
                                                                    >點擊更換</span
                                                                >
                                                            </div>
                                                        </v-img>
...
const { data: apiResponse, pending, error, refresh } = useFetch<ApiResponse>(`/api/restaurants/${storeId}`);
watch(
    apiResponse,
    (newApiResponse) => {
...
const saveStore = async () => {
    ...
    savePromises.push($fetch(`/api/admin/restaurants/${storeId}`, { method: "PATCH", headers, body: storeFormData }));
    ...
    for (const item of editableStore.value.menu) {
        ...
        if (item._id) {
            savePromises.push(
                $fetch(`/api/admin/restaurants/${storeId}/menu/${item._id}`, {
                    method: "PATCH",
                    headers,
                    body: menuFormData,
                }),
            ...
            savePromises.push(
                $fetch(`/api/admin/restaurants/${storeId}/menu`, { method: "POST", headers, body: menuFormData }),
            );
        }
    }
    ...
    for (const menuId of pendingDeletionMenuIds.value) {
        savePromises.push($fetch(`/api/admin/restaurants/${storeId}/menu/${menuId}`, { method: "DELETE", headers }));
    }
...
@media (max-width: 600px) {
    .sticky-actions {
        box-shadow: 0 -3px 10px -6px rgba(0, 0, 0, 0.18);
    }
    .section-title {
        font-size: 1.05rem;
    }
