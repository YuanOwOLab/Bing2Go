<template>
    <v-container fluid>
        ...
        <v-row v-else-if="error || !userInfo">
            <v-col cols="12">
                <v-alert type="error" title="無法載入使用者"> 找不到該使用者或發生伺服器錯誤。 </v-alert>
            </v-col>
        </v-row>
        ...
        <v-row v-else justify="center">
            <v-col cols="12" md="4" lg="3">
                <v-card elevation="2" class="text-center py-6" height="auto">
                    ...
                    <v-chip :color="roleColor" class="font-weight-bold mb-6" label>
                        {{ roleDisplay }}
                    </v-chip>
                            ...
                            <v-col cols="12" sm="6" md="6" class="pb-4">
                                <div class="d-flex align-start">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-body-1 font-weight-medium mt-1 text-wrap">
                                            {{ userInfo.address || "未填寫" }}
                                        </div>
                                    </div>
                                </div>
                            </v-col>
...
const {
    ...
    pending,
    error,
} = await useFetch<ApiResponse>(`/api/admin/users/${userId}`, {
    headers: {
