...
export default defineNuxtPlugin((nuxtApp) => {
    ...
    const connectToOrder = (orderId: string) => {
        ...
        ws.onmessage = (event) => {
            try {
                ...
                // 檢查是否在該訂單的狀態頁面
                const isCurrentOrderState =
                    currentRoute.path.includes(`/order-state/${orderId}`) ||
                    currentRoute.path.includes(`/customer-order-state/${orderId}`);
    ...
    watch(
        () => [userStore.token, userStore.currentRole],
        async ([token, role]) => {
            if (token && role) {
                try {
                    ...
                    if (data && data.data) {
                        const activeOrders = data.data.filter(
                            (o: any) => o.customerStatus !== "completed" && o.customerStatus !== "cancelled",
                        );
