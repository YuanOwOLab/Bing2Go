    ...
    <v-card flat border rounded="lg">
        <v-card-text class="pa-3 pa-sm-5">
            ...
            <div v-else-if="role === 'delivery'">
                ...
                <div class="text-right text-h6 font-weight-bold text-success">報酬: {{ order.deliveryFee }} 元</div>
            </div>
        </v-card-text>

        <v-card-actions v-if="!(role === 'delivery' && order.status === 'completed')" class="pa-3 pa-sm-4 pt-0">
            <div class="d-flex w-100" style="gap: 12px">
                <v-badge :model-value="hasNotification" color="error" dot class="notification-badge flex-grow-1">
                    <v-btn :to="path" color="primary" class="w-100" size="large" variant="flat">
...
const itemSummary = computed(() => {
    ...
    return props.order.items.map((item) => `${item.name} x ${item.quantity}`).join(", ");
});
